        ...
        <div className="min-h-[80vh] flex items-center justify-center px-4">
              <div className="w-full max-w-md">
                      <div className="text-center mb-8">
                                <h1 className="text-3xl font-bold text-gray-900">{isSignUp ? "Create your account" : "Welcome back"}</h1>h1>
                                <p className="text-gray-600 mt-2">{isSignUp ? "Start saving your work and unlock Pro features." : "Sign in to access your dashboard and saved projects."}</p>p>
                      </div>div>
                      ...
                      <div>
                        ...
                        {error && <p className="text-red-600 text-sm">{error}</p>p>}
                                <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium hover:bg-blue-700 transition">{isSignUp ? "Create Account" : "Sign In"}</button>button>
                      </form>form>
                      <p className="text-center text-sm text-gray-600 mt-6">
                        {isSignUp ? "Already have an account?" : "Don't have an account?"}{" "}
                                <button onClick={() => setIsSignUp(!isSignUp)} className="text-blue-600 font-medium hover:underline">{isSignUp ? "Sign in" : "Create one for free"}</button>button>
                      </p>p>
                      <div className="text-center mt-4">
                                <Link href="/tools" className="text-sm text-gray-500 hover:underline">Or continue without an account</Link>Link>
                      </div>div>
              </div>div>
        </div>div>
      );
